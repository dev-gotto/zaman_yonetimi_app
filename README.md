...
### ✅ 5. Aşama — İstatistik, kategori yönetimi, cilalama (TAMAMLANDI)
...
6. ✅ **Task Update (düzenleme) — kod tarafında tamamlandı:**
   ...
   - ⚠️ **Artık geçersiz — orijinal `titleController.dispose()` yaklaşımı:** Bu maddede önceden "Diyalog kapandığında `titleController.dispose()` çağrısı eklendi (`showDialog(...).then(...)`)" yazıyordu. Bu yaklaşım gerçek cihazda bir çökmeye yol açtığı için kaldırıldı — **bkz. aşağıdaki madde 8, "Diyalog Controller Çökmesi".** Güncel durum: controller artık `_TaskFormDialog` adlı ayrı bir `StatefulWidget`'ın kendi `initState`/`dispose()`'una bağlı.
   ...
   - **✅ Cihaz testi sonucu (25 Temmuz 2026, `flutter run` ile):** Kullanıcı test etti — konsolda `GUNCELLE BUTONUNA BASILDI` → `GOREV GUNCELLENIYOR` → `scheduleTaskNotification CAGRILDI` → `zonedSchedule BASARIYLA cagrildi` → `updateTask BASARIYLA TAMAMLANDI` akışı sorunsuz tamamlandı, yeni `dueDate`'e göre bildirim doğru şekilde yeniden planlandı. **Sonuç (o an için): Task Update production-ready kabul edildi.**
   - ⚠️ **Bu sonuç eksikti — bkz. madde 8.** Sonraki bir testte (28 Temmuz 2026) aynı ekranda gerçek bir çökme bulundu; kök neden yukarıdaki `titleController.dispose()` yaklaşımıydı. Düzeltme sonrası yeniden test edildi ve temiz çıktı — nihai "production-ready" durumu madde 8'in sonundadır.
7. ✅ **İstatistik ekranı + Ayarlar menüsü + yapısal refactor — kod tarafında tamamlandı:**
   ...
   - **✅ Cihaz testi sonucu (31 Temmuz 2026, `flutter run` ile):** Kullanıcı test etti — 3 sekme (Görevler/Takvim/Sayıç) sorunsuz açılıyor, Ayarlar menüsünde "Kategorileri Yönet" ve "İstatistikler" ikisi de doğru açılıyor, istatistik ekranındaki sayılar (toplam/tamamlanan/bekleyen + kategori dağılımı) gerçek görev sayılarıyla eşleşiyor. Aynı oturumda görev ekleme/güncelleme de tekrar test edildi, tamamen temiz log. **Sonuç: production-ready kabul edildi, checklist tam olarak kapandı.**
8. ✅ **Diyalog Controller Çökmesi — Bulundu ve Düzeltildi (28 Temmuz 2026):**
   - **Nasıl bulundu:** Kullanıcı Task Update'i cihazda ekran görüntüsü paylaşarak test etti — güncelleme verisi doğru kaydediliyordu (uygulama kapatılıp açılınca güncel veri görünüyordu) ama ekranda kırmızı hata ekranı çıkıyordu: `'_dependents.isEmpty': is not true.` Tam log istenip incelenince asıl hata ortaya çıktı: **`A TextEditingController was used after being disposed`** (`task_list_screen.dart`, görev formundaki `titleController`). `_dependents.isEmpty` ve "Tried to build dirty widget in the wrong build scope" hataları bunun **ardıl** sonuçlarıydı, kök neden değildi.
   - **Kök neden:** `titleController.dispose()`, `showDialog()`'un döndürdüğü Future'a (`.then(...)`) bağlıydı. Görev güncellenince `dueDate` değiştiği için liste yeniden sıralanıyor, bu da diyalog kapanış animasyonu sürerken paylaşılan bir üst context (`ProviderScope`) üzerinden diyaloğun yeniden build edilmesine yol açıyordu — controller o ana kadar zaten `.then()` ile disposed olmuş oluyordu.
   - **İlk düzeltme (kısmi):** `task_list_screen.dart`'taki form içeriği `_TaskFormDialog` adında ayrı bir `StatefulWidget`'a taşındı — `titleController` artık kendi `initState`/`dispose()`'una bağlı. Bu, görev formundaki çökmeyi giderdi. O sırada kategori diyaloglarındaki (`category_management_screen.dart`) controller'lara da hijyen amaçlı `.whenComplete(() => controller.dispose())` eklendi — **ama bu, aynı hatalı deseni tekrarlıyordu** (dispose hâlâ dış bir Future'ın tamamlanma anına bağlıydı, sadece `.then()` yerine `.whenComplete()` kullanıyordu).
   - **İkinci bulgu:** Bir sonraki cihaz testinde (aynı gün), bu kez `category_management_screen.dart:104`'teki "Yeniden Adlandır" diyaloğunda **aynı çökme** tekrar oluştu — bu, kök nedenin "liste `dueDate`'e göre sıralanıyor" olmadığını, daha genel olarak **"dispose'un dış bir Future'a bağlı olması + kapanış animasyonu sürerken herhangi bir Riverpod state güncellemesinin paylaşılan üst context üzerinden diyaloğu yeniden build etmeye zorlaması"** olduğunu doğruladı.
   - **Kesin düzeltme:** Yeni ortak bileşen `lib/features/categories/category_name_dialog.dart` (`CategoryNameDialog`, `ConsumerStatefulWidget`) — controller'ı kendi `initState`/`dispose()`'una bağlı, dış bir Future zamanlamasına **hiç** bağlı değil. Hem `category_management_screen.dart`'taki ekleme/yeniden adlandırma hem de `task_list_screen.dart`'taki (`_TaskFormDialog` içindeki) iç içe "yeni kategori" akışı artık bu tek bileşeni kullanıyor — kod tekrarı da azaldı, aynı hata dördüncü bir yerde tekrar yazılamaz hale geldi.
   - **✅ Nihai cihaz testi sonucu (28 Temmuz 2026, iki ayrı test, `flutter run` ile):** Görev güncelleme art arda iki kez denendi, tamamen temiz log (`GUNCELLE BUTONUNA BASILDI` → `updateTask BASARIYLA TAMAMLANDI`, hiç exception yok). **Sonuç: Task Update GERÇEKTEN production-ready — bu sefer hem görev formu hem kategori diyalogları cihazda doğrulandı.**
   - **Ders (bkz. Çalışma Tarzı Tercihleri'ndeki ilgili madde):** Bir `TextEditingController`'ın `dispose()`'u asla dış bir `Future`'ın tamamlanma anına (`.then()`, `.whenComplete()`, `try/finally` içinde bir `await` zincirinin sonu) bağlanmamalı — controller'ı kullanan widget kendi `StatefulWidget`'ı olmalı, dispose widget'ın kendi yaşam döngüsüne bağlı olmalı.
9. **Sonraki adım (YENİ SOHBETTE BURADAN BAŞLA):** Proje roadmap'indeki tüm ana aşamalar tamamlandı, Task Update çökmesi kalıcı olarak düzeltildi VE İstatistik/Ayarlar navigasyon checklist'i tam olarak doğrulandı (31 Temmuz 2026) — **açık/yarım kalan bilinen bir iş yok.** Sıradaki iş kullanıcıyla birlikte belirlenecek — adaylar: UI cilalaması, gerçek "tekrarlayan görev" özelliği (bkz. "Kullanılmayan" listesindeki `RepeatType` notu), debug print temizliği, ya da yeni bir özellik.
...
- **⚠️ TextEditingController ömrü — dış Future'a değil, widget yaşam döngüsüne bağlanacak:** Bir controller'ı `dispose()` ederken bunu asla `showDialog(...).then(...)`, `.whenComplete(...)` veya bir `await` zincirinin sonundaki bir `finally` bloğuna bağlama — bunların hepsi aynı yarış durumu ailesine giriyor (dispose, widget hâlâ ağaçtaysa ve bir üst context rebuild oluyorsa, gerçek unmount'tan ÖNCE tetiklenebiliyor). Bunun yerine controller'ı tutan içeriği kendi `initState`/`dispose()`'una sahip bir `StatefulWidget` yap. Bu proje bunu üç ayrı yerde (görev formu, kategori ekleme, kategori yeniden adlandırma) atlayıp üç kez aynı çökmeyi yaşadı — bkz. 5. Aşama, madde 8 için tam hikâye. Kesin çözüm için bkz. `lib/features/categories/category_name_dialog.dart`.
